...
- **What the guide calls the programs.** Each channel picks, on its row in the control room: the file names (cleaned up, so `02 Design for Dreaming (1956).mp4` reads DESIGN FOR DREAMING (1956)), the titles written inside the files (what `ffmpeg -metadata title=` or HandBrake put there, read once and cached in `.titles.json` beside the videos), or one fixed name for the whole channel. Name files in airing order either way; the sort is by file name.
